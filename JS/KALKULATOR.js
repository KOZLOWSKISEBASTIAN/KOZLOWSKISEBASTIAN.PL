(function () {
	...
	function isTypingDecimal(str) {
		var s = String(str);
		var dot = s.indexOf('.');
		if (dot === -1) return false;
		var frac = s.slice(dot + 1);
		return s.endsWith('.') || frac.length < 3;
	}
	...
	function buildDisplayText() {
		...
		if (operator) {
			if (waitingSecond) {
				...
				var curStr = value;
				var curDisp = isTypingDecimal(curStr)
					? format(curStr)
					: formatNumber(parseFloat(curStr));
				return formatNumber(first) + ' ' + (sym[operator] || operator) + ' ' + curDisp;
			}
		}
		return format(value);
	}
